      ...
      <div class="flex h-full min-h-0 flex-col overflow-hidden">
        <div class="shrink-0 border-b border-default bg-muted/80 px-4 py-2.5">
          <div class="flex flex-col gap-2">
            ...
            <UTabs v-if="conflictCount > 0 || allowAdditions" v-model="editorTab" :content="false" value-key="value"
              variant="link" color="primary" :items="[
            ...
            <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted">
              ...
              <span v-if="conflictCount > 0" class="text-default">{{ resolvedCount }}/{{ conflictCount }}
                resolved</span>
              ...
              <div class="ml-auto flex items-center gap-3">
                ...
                <div class="flex gap-1">
                  <UButton size="xs" variant="outline" :color="mergeResultLayout === 'right' ? 'primary' : 'neutral'"
                    label="Right" @click="mergeResultLayout = 'right'" />
                  <UButton size="xs" variant="outline" :color="mergeResultLayout === 'bottom' ? 'primary' : 'neutral'"
                    label="Bottom" @click="mergeResultLayout = 'bottom'" />
                </div>
                <LangThemeSelect />
              </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3 border-t border-default pt-2">
              ...
              <div v-if="editorTab === 'conflicts' && conflictCount > 0"
                class="flex flex-1 flex-wrap items-center justify-center gap-2">
                <UButton size="sm" variant="outline" label="Prev" :disabled="currentConflictNum <= 1"
                  @click="currentConflictNum -= 1" />
                <UButton size="sm" variant="outline" label="Next" :disabled="currentConflictNum >= conflictCount"
                  @click="currentConflictNum += 1" />
                <span class="text-sm font-medium tabular-nums text-default">Conflict {{ currentConflictNum }} of {{
                  conflictCount }}</span>
                <UBadge :color="choiceBadge.color" variant="subtle">{{ choiceBadge.text }}</UBadge>
                <UButton size="sm" :color="currentChoice === 'A' ? 'primary' : 'neutral'"
                  :variant="currentChoice === 'A' ? 'solid' : 'outline'" :label="`Choose ${labelA}`"
                  @click="choose(currentChunkIndex, 'A')" />
                <UButton size="sm" :color="currentChoice === 'B' ? 'secondary' : 'neutral'"
                  :variant="currentChoice === 'B' ? 'solid' : 'outline'" :label="`Choose ${labelB}`"
              ...
              <div v-else-if="editorTab === 'additions' && addedCount > 0"
                class="flex flex-1 flex-wrap items-center justify-center gap-2">
                <UButton size="sm" variant="outline" label="Prev" :disabled="currentAdditionNum <= 1"
                  @click="currentAdditionNum -= 1" />
                <UButton size="sm" variant="outline" label="Next" :disabled="currentAdditionNum >= addedCount"
                  @click="currentAdditionNum += 1" />
                <span class="text-sm font-medium tabular-nums text-default">Addition {{ currentAdditionNum }} of {{
                  addedCount
                  }}</span>
              ...
              <div v-else class="flex-1 text-center text-sm text-muted">
                ...
                <template v-else-if="addedCount > 0 && !allowAdditions">No shared-key conflicts — {{ addedCount }}
                  additions from {{ labelB }} will be appended</template>
                <template v-else-if="addedCount === 0 && conflictCount === 0">Files are identical — nothing to
                  merge</template>
              </div>

              <UButton class="shrink-0" size="sm" color="primary" label="Save & Continue"
                :disabled="conflictCount > 0 && resolvedCount < conflictCount" @click="save" />
            </div>
          </div>
        </div>
        ...
        <div class="min-h-0 flex-1 overflow-hidden">
          <template
            ...
            <SplitPane :orientation="mergeResultLayout === 'right' ? 'horizontal' : 'vertical'"
              :default-second-size="mergeResultLayout === 'right' ? 480 : 300" fixed-side="second"
              ...
              <template #first>
                <DiffView
                  ...
                  :original-label="editorTab === 'additions' ? '(not in A)' : labelA" :modified-label="labelB"
              ...
              <template #second>
                <EditorView :content="resultDisplay" label="Result" label-class="bg-accent/10 text-accent"
                  :first-line-number="(editorTab === 'additions' ? additionsDiffDisplay : diffDisplay)?.startLineA ?? 1"
                  :read-only="editorTab === 'additions'" class="h-full" @content-change="onResultChange" />
              </template>
            </SplitPane>
          </template>
          ...
          <div v-else class="flex h-full flex-col items-center justify-center gap-3 text-muted">
            <template v-if="conflictCount === 0 && addedCount > 0">
              ...
              <span class="text-sm">{{ addedCount }} entry additions from {{ labelB }} will be appended to the merged
                output.</span>
            </template>
