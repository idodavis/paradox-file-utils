<template>
  <div v-if="visible" class="fixed inset-0 bg-dark-bg/95 backdrop-blur-sm z-[60] flex flex-col"
    ...
    <div
      ...
      <div class="flex flex-col px-6 py-4 border-b border-dark-border bg-dark-panel/50">
        ...
        <div class="flex items-center gap-2">
          <input ref="searchInput" v-model="searchQuery" @input="performSearch" @keydown.enter.prevent="nextMatch"
            ...
            class="flex-1 px-3 py-2 bg-dark-input/80 border border-dark-border rounded-lg text-gray-200 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-btn-primary/50 focus:border-btn-primary transition-all duration-200" />
          <span v-if="searchQuery && searchMatches.length > 0" class="text-sm text-gray-400 whitespace-nowrap">
            {{ currentMatchIndex + 1 }} / {{ searchMatches.length }}
          </span>
          <button v-if="searchQuery" @click="clearSearch"
            class="px-3 py-2 bg-gray-600/80 hover:bg-gray-700/80 text-white rounded-lg text-sm font-medium transition-all duration-200">Clear</button>
          <button v-if="searchQuery && searchMatches.length > 0" @click="prevMatch"
            class="px-3 py-2 bg-gray-600/80 hover:bg-gray-700/80 text-white rounded-lg text-sm font-medium transition-all duration-200">↑
            Prev</button>
          <button v-if="searchQuery && searchMatches.length > 0" @click="nextMatch"
            class="px-3 py-2 bg-gray-600/80 hover:bg-gray-700/80 text-white rounded-lg text-sm font-medium transition-all duration-200">Next
            ↓</button>
        </div>
      </div>
