<template>
  <div class="flex flex-col h-screen bg-dark-bg text-gray-200">
    <header class="bg-dark-panel px-8 py-4 border-b border-dark-border flex justify-between items-center">
      <h1 class="text-3xl font-semibold">Paradox Script Tools</h1>
    ...
    <main class="flex-1 grid grid-cols-2 gap-4 p-4 overflow-auto bg-dark-input">
      <!-- Configuration Panel -->
      <div v-if="!showComparisonTool" class="bg-dark-panel rounded-lg p-6 overflow-y-auto">
        <!-- Merge Options -->
        <div class="mb-6">
          <div class="mb-4">
      ...
      <!-- Results Panel -->
      <div v-if="!showComparisonTool && mergeResults.length > 0" class="bg-dark-panel rounded-lg p-6 overflow-y-auto">
        ...
        <div v-for="(result, index) in mergeResults" :key="index" class="p-4 bg-dark-input rounded mb-2">
          ...
          <button @click="viewDiff(index, result.FileAPath, result.OutputPath)"
            class="px-4 py-2 bg-btn-primary hover:bg-btn-primary-hover text-white rounded font-medium transition-colors">
            View Diff: {{ result.FileAPath }} -> {{ result.OutputPath }}
          </button>
          <button @click="viewDiff(index, result.FileBPath, result.OutputPath)"
            class="px-4 py-2 bg-btn-primary hover:bg-btn-primary-hover text-white rounded font-medium transition-colors">
            View Diff: {{ result.FileBPath }} -> {{ result.OutputPath }}
          </button>
        </div>
      </div>

      <!-- Diff Panel -->
      <div v-if="currentDiff && !showComparisonTool" class="col-span-2 bg-dark-panel rounded-lg p-6">
