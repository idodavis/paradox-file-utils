...
// Comment is a `#` comment; Text includes the leading `#`; Line is 0-based.
type Comment struct {
	Text  string
	Range Range
	Line  int
...
// WalkStatements does a depth-first walk over every statement in the tree,
// invoking fn for each. Returning false from fn stops descent into that
// statement's child block.
func WalkStatements(root *Root, fn func(st Statement) bool) {
	var visit func(stmts []Statement)
