...
// and falls back to a latin1 decode for files that are not valid UTF-8.
...
// BOM and returns hadBOM. If the bytes are not valid UTF-8 (typically hand-edited
// Latin-1/Windows-1252 files) it falls back to a latin1 decode so callers get
// usable text rather than U+FFFD replacement characters.
func Decode(raw []byte) (text string, hadBOM bool) {
	...
	if utf8.Valid(body) {
		return string(body), hadBOM
	}
	return latin1Decode(raw), false
}
